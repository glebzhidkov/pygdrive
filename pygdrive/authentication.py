...
def authenticate(
    ...
    if client_secret:
        ...
        raise ValueError(
            "Please specify `client_secret` or `service_account_secret` or `creds`."
        )
    ...
    if creds_env_var and creds_env_var in os.environ:
        try:
            ...
            creds = credentials.Credentials.from_authorized_user_info(
                info=json.loads(env_value), scopes=scopes
            )
        except:
            warnings.warn(
                f"Failed at using previously generated credentials stored in the environment variable {creds_env_var}"
            )
        ...
        try:
            ...
            warnings.warn(
                f"Failed at using previously generated credentials stored in the file {creds_path}"
            )
            creds = __oauth2_auth_new(client_secret=client_secret, scopes=scopes)
    else:
        creds = __oauth2_auth_new(client_secret=client_secret, scopes=scopes)
...
def _service_account_auth(service_account_secret, scopes) -> DriveClient:
    ...
    if service_account_secret in os.environ:
        creds = service_account.Credentials.from_service_account_info(
            info=json.loads(os.environ[service_account_secret]), scopes=scopes
        )
    elif os.path.exists(service_account_secret):
        creds = service_account.Credentials.from_service_account_file(
            filename=service_account_secret, scopes=scopes
        )
    else:
